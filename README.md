...
These appear below the toolbar when applicable:

| Button | Action |
|---|---|
| compute S/2 | Replace generators with those of S/2 = { x : 2x ∈ S } |
| add all PF ≠ f | Add every pseudo-Frobenius number other than f as a generator (shown when t > 1) |
| add reflected gaps | Add all reflected gaps as generators |
| self glue | Self-gluing: compute m·S + x·S where x is the first generator with gcd(m, x) = 1 (shown when such x exists) |
