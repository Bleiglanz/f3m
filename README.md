...
| Ascent | Structural reverse of Descent. The interval (μ, f) splits into **strata** of width m: stratum l = (f−(l+1)m, f−l m) for l = 0, 1, …, level−1 (l = 0 is V(S)). For each (l, k) with k = 1, …, level, picks the largest atom a such that k·a is an Apéry element of S landing in stratum l, or k·a = f+m. Toggles a (or runs the f+m removal when k·a = f+m). Examples: ⟨4, 5⟩ fires at l=0, k=2 (2·5 = 10 ∈ V(S)); ⟨4, 9⟩ fires at l=1, k=2 (2·9 = 18 in the next stratum). Empirically Δg = +1 in every firing case. No-op when no (l, k) matches |
