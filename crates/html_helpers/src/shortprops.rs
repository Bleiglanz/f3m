...
///
/// Columns: m, f, es, e, σ, g, rl, t, r, ra, ρ, fg, Sym, V⊆S, Wilf, 1/e, piv — in that order.
...
pub fn shortprop_cells(sg: &Semigroup) -> String {
    ...
    format!(
        "<td>{m}</td><td>{f}</td><td>{es}</td>{e_td}<td>{cg}</td><td>{g}</td>\
         ...
         <td>{sym}</td><td>{vins}</td><td>{wilf:.4}</td><td>{inv_e:.4}</td><td>{pivot}</td>",
        m = sg.m,
...
pub fn shortprop(sg: &Semigroup) -> String {
    format!(
        "<table class=\"shortprop-table\"><thead><tr>\
         ...
         <th title=\"Small minimal generators: count of minimal generators g with g &lt; f\u{2212}m\">es</th>\
         ...
         <th title=\"Large reflected gaps: gaps L with f\u{2212}m &lt; L &lt; f (automatically reflected)\">rl</th>\
         ...
         <th title=\"Pivot: largest pseudo-Frobenius number below f (or f itself when there is none)\">piv</th>\
