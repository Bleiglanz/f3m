...
pub fn basic(s:&Semigroup){
    ...
    if s.m>1 {assert_eq!(s.diag(s.mu), s.r)};
    ...
    assert_eq!(s.apn.len(),s.pho+1);
    let apn_rsum:usize = (0..s.pho+1).map(|x|s.apn[x]*x).sum();
    ...
    if s.mu== add_f_to_semigroup.mu &&  s.m== add_f_to_semigroup.m && s.m>2{
        assert_eq!(s.pho+1, add_f_to_semigroup.pho);
        for i in 1..s.pho+1 {
            if i<s.apn.len()  {
                assert_eq!(s.apn[i], add_f_to_semigroup.apn[i + 1], "{:?} vs {:?} for {gens:?} at i={i}", s.apn, add_f_to_semigroup.apn);
            }
        }
