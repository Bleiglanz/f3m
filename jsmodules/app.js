...
function buildCsv() {
  const CSV_HEADER = '#,toggle,m,f,es,e,σ,g,rl,t,r,ra,ρ,fg,Sym,V⊆S,Wilf,1/e,expr,value,⊆?,generators';
...
// Signed change of two numeric values, used by the delta-info banner.
...
function renderPivotDetail(filters, pivotData) {
  ...
  const cols = ['g', 'm', 'es', 'e', 'type', 'f', 'rl', 'r', 'sigma', 'gen', 'pf'];
...
function update3dButtonState() {
  ...
  if (_running3d) {
    ...
    // Ascent self-terminates when it becomes a no-op; enable whenever we have a semigroup.
    ascBtn.disabled = !currentS;
  }
}
