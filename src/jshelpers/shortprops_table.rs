...
/// Render the nine data `<td>` cells shared by the compact summary row and history table rows:
/// m, f, e, g, c-g, t, Sym, gen, PF — in that order.
pub(super) fn shortprop_cells(sg: &Semigroup) -> String {
    ...
    format!(
        "<td>{m}</td><td>{f}</td><td>{e}</td><td>{g}</td><td>{cg}</td>\
         <td>{t}</td><td>{sym}</td>\
         <td class=\"left\">{atoms}</td><td class=\"left\">{pf}</td>",
        m = sg.m,
